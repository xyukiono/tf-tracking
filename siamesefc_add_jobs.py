...
class HyperParams(HyperParamsBase):
    def __init__(self):
        ...
        self.script_name = 'siamesefc_train.py'
...
def get_command(params, log_dir):
    ...
    CMD += ' --adjust_response_config_scale={}'.format(params.adjust_response_config_scale)
...
if __name__ == '__main__':
    ...
    # Basic test
    memo = 'long-long training'
    pg.add_params('sub_dir', '180710-siamesefc-basic')
    pg.add_params('optim_method', ['momentum'])
    pg.add_params('lr_decay', [True, False])
    pg.add_params('weight_decay', ['5e-4', '0'])

    # pg.add_params('optim_method', ['adam'])
    # pg.add_params('lr', ['1e-3', '1e-4'])
